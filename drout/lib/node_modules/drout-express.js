...
const DroutPath = require("drout-path");
  ...
  render: (options) => {
    return ((request, response, next) => {
      if (/^\/drout\.js$/.test(request.path)) {
        ...
        const transformedContent = JSON.stringify(bundleOptions).replace(/"componentId":"([^"]*)"/g, 'component: require("$1")');
        const clientCode = `
          Drout = require("drout");
          React = Drout.React;
        ...
        `;
        // TODO maybe we should figure out a way to prefer the parent project's
        // node_modules if multiple node_modules with the same name are defined?
        return browserifyString(clientCode, { paths: module.paths })
          .bundle()
          .pipe(response);
      }
      // TODO we need to do better path matching than direct match!
      if (options.paths[request.path]) {
        const pageOptions = Object.assign({}, options.paths[request.path]);
      ...
      if (request.path === '/drout-config-store') {
        ...
        if (options.inherited && options.inherited["drout-config-store"]) {
          // TODO parse post
          options.inherited["drout-config-store"](request.params);
